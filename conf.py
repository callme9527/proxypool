...
validate_p_num = 4  # 验证的进程数
greelet_num = 10  # 每个进程的最大协程数
p_max_live = 60 * 5  # 进程最大存活时间
validate_wait_time = 2  # 验证进程数达到最大时的等待时间
join_max_time = 60 * 15


store_num_per = 10  # 一次存储的可用代理数
update_num_per = 10  # 每次更新代理的数目

init_score = 5  # 每个可用代理的初始分数 0则删除
raw_init_score = 3  # 每个原始代理的初始分数 0则删除

proxy_canuse_url = 'https://www.baidu.com'
get_ip_url = 'http://httpbin.org/ip'
proxy_type_url = 'httpbin.org/get'
proxy_area_url = 'https://sp0.baidu.com/8aQDcjqpAAV3otqbppnN2DJv/api.php?query=%s&resource_id=6006'

# 代理网址及获取方式
url_pats = [
    {
        'urls': ['http://www.ip181.com/daili/%d.html' % id for id in range(1, 6)],
        'pat':{
            'type': 'xpath',
            'match': './/table/tbody/tr[position()>1]',
            'pos': ['./td[1]/text()', './td[2]/text()']
        }
    },
    {
        'urls': ['http://www.xicidaili.com/nn/%d' % id for id in range(1, 16)],
        'pat':{
            'type': 'xpath',
            'match': './/table[@id="ip_list"]/tr[position()>1]',
            'pos': ['./td[2]/text()', './td[3]/text()']
        }
    },
    {
        'urls': ['http://www.kuaidaili.com/free/inha/%d' % id for id in range(1, 21)],
        'pat':{
            'type': 'xpath',
            'match': './/tbody/tr',
            'pos': ['./td[1]/text()', './td[2]/text()']
    ...
    {
        'urls': ['http://www.66ip.cn/%d.html' % id for id in range(1, 16)],
        'pat':{
            'type': 'xpath',
            'match': './/table/tr[position()>1]',
            'pos': ['./td[1]/text()', './td[2]/text()']
        }
    },
    {
        'urls': ['http://www.goubanjia.com/index%d.shtml' % id for id in range(1, 16)],
        'pat':{
            'type': 'xpath',
            'match': './/div[@id="list"]/table/tbody/tr',
            'pos': ['./td[1]/*[@style]/text()', './td[1]/span[last()]/text()']
        }
    },
    {
        'urls': ['http://www.mimiip.com/gngao/%d' % id for id in range(1, 16)],
        'pat':{
            'type': 'xpath',
            'match': './/table[@class="list"]/tr[position()>1]',
            'pos': ['./td[1]/text()', './td[2]/text()']
        }
    },
    {
        'urls': ['http://www.httpsdaili.com/?stype=3&page=%d' % id for id in range(1, 6)],
        'pat': {
            'type': 'xpath',
            'match': './/tbody[1]/tr',
            'pos': ['./td[1]/text()', './td[2]/text()']
